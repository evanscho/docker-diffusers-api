...
async def send_status_update(process_name: str, status: str, payload: dict = {}, options: dict = {}):
    ...
    if status == "start":
        session[process_name] = {"start": current_time, "last_time": current_time}
    elif status == "done":
        if process_name in session and "start" in session[process_name]:
            session[process_name].update({
                "done": current_time,
                "diff": current_time - session[process_name]["start"]
            })
        else:
            logging.warning(f'Process "{process_name}" does not have a start time so cannot log the end time')
            return
    else:
